...
import {
  ...
  TreePine, Briefcase, Sparkles, ChevronDown, UserCheck,
} from 'lucide-react';
...
const CATEGORY_DEFINITIONS = [
  ...
  { id: 'asesor',       label: 'Asesores',       Icon: UserCheck  },
];
...
export default function Home({ session }) {
  ...
  // Carga ultra-rápida de Supabase
  ...
  // Filtrar Desarrollos/Landings locales según búsqueda y categoría
  const desarrollosFiltrados = useMemo(() => {
    return DESARROLLOS.filter(dev => {
      const matchesCat = active === 'todas' || 
        (active === 'asesor' && dev.tipo === 'asesor') ||
        (active !== 'asesor' && dev.tipo === active);
      const matchesSearch = !q.trim() || 
        dev.titulo.toLowerCase().includes(q.toLowerCase()) || 
        dev.ciudad.toLowerCase().includes(q.toLowerCase()) ||
        dev.descripcion.toLowerCase().includes(q.toLowerCase());
      return matchesCat && matchesSearch;
    });
  }, [active, q]);

  // Conteo de elementos disponibles por categoría (DB + Landings)
  const categoryCounts = useMemo(() => {
    const counts = { todas: dbTotal + DESARROLLOS.length };

    // Contar Landings por tipo
    DESARROLLOS.forEach(d => {
      const t = d.tipo || 'terreno';
      counts[t] = (counts[t] || 0) + 1;
    });

    // Contar Propiedades de DB por tipo aproximado
  ...
  // REGLA SOLICITADA: Ocultar botones de categoría que NO tengan elementos
  ...
  const totalItemsCount = desarrollosFiltrados.length + (active === 'asesor' ? 0 : propiedades.length);
  ...
  return (
    <>
      ...
      <Helmet>
        ...
        <meta name="description" content="Encuentra casas, terrenos, desarrollos y locales comerciales en Chiapas. El portal inmobiliario #1 de Chiapas." />
      ...
      <div className="pec">
        ...
        {/* Categorías Tipo iOS — Solo visibles si existen propiedades/landings en esa categoría */}
        ...
        <main className="pec-wrap">
            ...
            <div className="pec-toolbar">
              <p className="pec-count">
                {hayFiltros
                  ...
                  : <><b>{(dbTotal + DESARROLLOS.length).toLocaleString('es-MX')}</b> opciones disponibles en Chiapas</>
                }
              </p>
              {hayFiltros && (
          ...
          {loading ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'6rem 1rem', gap:'1rem', color:'#64748b' }}>
              <div className="pec-spinner" />
              <p style={{ fontWeight:600, fontSize:14 }}>Cargando portal...</p>
            </div>
          ) : totalItemsCount === 0 ? (
            <div className="pec-empty">
              ...
              <button
                onClick={limpiar}
                style={{ background:'#1e293b', color:'#fff', border:'none', borderRadius:999, padding:'12px 28px', fontWeight:700, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}
              >
            ...
            <>
              {/* ── Fichas de Landings / Desarrollos (alimentadas con todas sus imágenes) ── */}
                  ...
                  <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', gap:12, marginBottom:'1rem' }}>
                    <div>
                      <h2 style={{ fontSize:'1.15rem', fontWeight:800, color:'#1e293b', letterSpacing:'-.3px' }}>
                        {active === 'asesor' ? 'Asesores Inmobiliarios IBR' : 'Desarrollos & Landings Destacadas'}
                      </h2>
                      <p style={{ fontSize:'.85rem', color:'#64748b', marginTop:2 }}>
                        {active === 'asesor' ? 'Asesores con respaldo profesional y atención personalizada' : 'Proyectos con financiamiento directo y atención inmediata'}
                      </p>
                    </div>
                  </div>
              ...
              {active !== 'asesor' && propiedades.length > 0 && (
...
console.log('Successfully updated Home.jsx —', homeCode.split('\\n').length, 'lines');
