...
export default function Home({ session }) {
  ...
  const [selectedDev, setSelectedDev] = useState(null); // Modal de Ficha Inmobiliaria
  ...
  // Filtrar Desarrollos/Landings locales
  ...
  // REGLA: Ocultar categorías con conteo === 0
      ...
      <div className="pec">
        ...
        <main className="pec-wrap">
          ...
          {loading ? (
            ...
            <>
              {/* ── Fichas Inmobiliarias (Abrir Ficha al Hacer Clic) ── */}
                  ...
                  <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', gap:12, marginBottom:'1rem' }}>
                    <div>
                      <h2 style={{ fontSize:'1.15rem', fontWeight:800, color:'#1e293b', letterSpacing:'-.3px' }}>
                        {active === 'asesor' ? 'Asesores Inmobiliarios IBR' : 'Desarrollos & Fichas Destacadas'}
                      </h2>
                      <p style={{ fontSize:'.85rem', color:'#64748b', marginTop:2 }}>
                        {active === 'asesor' ? 'Asesores con respaldo profesional y atención personalizada' : 'Haz clic en cualquier tarjeta para abrir su ficha inmobiliaria completa'}
                      </p>
                    </div>
                  </div>
...
console.log('Successfully updated Home.jsx with DevelopmentDetailModal integration —', homeCode.split('\\n').length, 'lines');
