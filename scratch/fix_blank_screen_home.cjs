...
export default function Home({ session }) {
  ...
  const [loading,     setLoading]     = useState(false); // Carga inmediata sin pantalla blanca
  const [loadingMore, setLoadingMore] = useState(false);
  const [dbTotal,     setDbTotal]     = useState(0);
  const [page,        setPage]        = useState(0);
  const [hasMore,     setHasMore]     = useState(false);

  // Consulta Supabase ultra-segura con timeout para evitar cuelgues
  ...
  useEffect(() => {
    ...
    const fetchInit = async () => {
      try {
        ...
        const fetchPromise = buildQuery(0, PAGE_SIZE - 1);
  ...
  // Conteo de elementos disponibles por categoría
  ...
  // REGLA: Ocultar botones de categoría que NO tengan elementos
...
console.log('Successfully updated Home.jsx safely —', homeCode.split('\\n').length, 'lines');
