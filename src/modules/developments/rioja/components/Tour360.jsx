...
export default function Tour360({ onClose }) {
  ...
  const currentScene = rioja360Tour[currentIndex];
  ...
  useEffect(() => {
    ...
    viewerRef.current = new Viewer({
      ...
      defaultYaw: rioja360Tour[0].initialYaw || 0,
      defaultPitch: rioja360Tour[0].initialPitch || 0
    });
  ...
  useEffect(() => {
    ...
    if (viewerRef.current) {
      ...
      viewerRef.current.setPanorama(currentScene.file, {
  ...
  const handleHotspotClick = (hs) => {
    if (hs.type === 'navigation') {
      const idx = rioja360Tour.findIndex(s => s.id === hs.targetScene);
      if (idx !== -1) {
        setCurrentIndex(idx);
      }
    } else {
    ...
    <div className="rioja-360-overlay">
      ...
      <div className="rioja-360-header">
        <div className="rioja-360-title">
          {currentScene.title} <span className="rioja-360-counter">({currentIndex + 1} / {rioja360Tour.length})</span>
        </div>
      ...
      <div className="rioja-360-viewer-wrapper">
        ...
        <div className="rioja-360-minimap">
          <svg className="rioja-minimap-svg" viewBox="0 0 100 100">
            <line x1="30" y1="80" x2="50" y2="55" stroke="rgba(195, 164, 121, 0.4)" strokeWidth="1.5" strokeDasharray="3" />
            <line x1="50" y1="55" x2="70" y2="35" stroke="rgba(195, 164, 121, 0.4)" strokeWidth="1.5" strokeDasharray="3" />
            <line x1="70" y1="35" x2="80" y2="15" stroke="rgba(195, 164, 121, 0.4)" strokeWidth="1.5" strokeDasharray="3" />
          </svg>
        ...
        {selectedHotspot && (
          <div className="rioja-360-popup-card">
            ...
            <div className="rioja-popup-card-body">
              ...
              {selectedHotspot.type === 'action' && selectedHotspot.actionUrl && (
                <a 
                  href={selectedHotspot.actionUrl} 
                  target="_blank" 
                  rel="noreferrer" 
                  className="rioja-popup-card-action-btn"
                >
                  Continuar Acción <ArrowRight size={16} />
                </a>
              )}
            </div>
          </div>
        ...
        {/* Controles de Flechas Laterales Tradicionales */}
